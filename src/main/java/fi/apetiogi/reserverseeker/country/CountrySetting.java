...
public class CountrySetting extends Setting<Country> {
    ...
    public static void countrySettingW(WTable table, CountrySetting setting, GuiTheme theme) {
        ...
        reset.action = () -> {
            setting.reset();
            country.set(ReServerSeeker.COUNTRY_MAP.get("UN"));
            nameLabel.set("Any");
        };
    }
    ...
    protected Country parseImpl(String str) {
        return ReServerSeeker.COUNTRY_MAP.get(str);
    }

    @Override
    protected boolean isValueValid(Country value) {
        return value.code != null && ReServerSeeker.COUNTRY_MAP.containsValue(value);
    }
    ...
    protected Country load(NbtCompound tag) {
        return ReServerSeeker.COUNTRY_MAP.get(tag.getString("value"));
    }
